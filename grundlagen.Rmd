...
Die Funktion [topfeatures](https://docs.quanteda.io/reference/topfeatures.html) zählt Features in der gesamten DFM aus. 

```{r Einfache Worthäufigkeiten berechnen}
topfeatures(meine.dfm)
```

Die Funktion [textstat_frequency](https://docs.quanteda.io/reference/textstat_frequency.html) liefert zusätzlich zu dieser Angabe noch den Rang (rank), die Anzahl der Dokumente, in denen das Feature vorkommt (docfreq) sowie Metadaten, nach denen bei der Zählung gefiltert wurde. Grundsätzlich ist textstat_frequncy gegenüber topfeatures zu bevorzugen.
